...
// Set while the .closing animation (see #recording-multiselect-bar.closing
// in subjects.css) is in flight -- same role as recordingDropdownCloseHandler
// below, for the same reason (a fast reopen needs to cancel it cleanly).
let recordingMultiselectCloseHandler = null;
...
function updateRecordingMultiselectBar() {
    ...
    const isShown = recordingMultiselectBar.classList.contains("open");
    if (show === isShown) return; // already in the right state

    if (show) {
        // Opening: cancel any close animation still in flight and mirror
        // openRecordingSelectDropdown()'s own open (see .open in
        // subjects.css).
        if (recordingMultiselectCloseHandler) {
            recordingMultiselectBar.removeEventListener("animationend", recordingMultiselectCloseHandler);
            recordingMultiselectCloseHandler = null;
        }
        recordingMultiselectBar.classList.remove("closing");
        recordingMultiselectBar.classList.add("open");
    } else {
        // Closing: swap to .closing so its fade/scale-down animation
        // plays before it's actually hidden (see closeRecordingSelectDropdown()
        // for the identical pattern on the dropdown itself), instead of
        // popping away the instant the count/dropdown state above goes to 0.
        recordingMultiselectBar.classList.remove("open");
        recordingMultiselectBar.classList.add("closing");
        recordingMultiselectCloseHandler = () => {
            recordingMultiselectBar.classList.remove("closing");
            recordingMultiselectCloseHandler = null;
        };
        recordingMultiselectBar.addEventListener("animationend", recordingMultiselectCloseHandler, { once: true });
    }
}
...
// Two panels take turns occupying the empty pinboard area, based on how
// far the user's gotten toward an actual analysis target. The "Select
// Recording" pill/dropdown is always visible (Subjects -> Sessions ->
// Recordings drill-down lives inside the dropdown itself), so there's no
// separate landing panel for the "nothing picked yet" moment anymore:
//   - no analysis target set yet -> the mid-pinboard nudge message
//     (#pinboard-no-target-msg), regardless of whether a subject's
//     even been browsed to yet
...
//   - a target is set AND at least one widget is on the board -> neither
//     nudge message is shown.
...
function updatePinboardEmptyState() {
    ...
    if (noTargetMsg) noTargetMsg.classList.toggle("is-hidden", hasTarget);
...
function closeRecordingSelectDropdown() {
    ...
    syncMultiSelectFromTarget();
    // renderRecordings() calls updateRecordingMultiselectBar() internally,
    // which (now that the dropdown's own class above already reads as
    // closed) plays the bar's own closing animation rather than an
    // instant hide.
    renderRecordings();
